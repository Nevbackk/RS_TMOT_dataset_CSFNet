...
import sys
import os
# 添加项目根目录到 sys.path，以便导入 path_config
...
class COCO(data.Dataset):
    ...
    def __init__(self, opt, split):
        ...
        # 使用 path_config 中的路径
        self.img_dir = path_config.DATA_ROOT
        if split == 'train':
            self.annot_path = path_config.TRAIN_JSON
        elif split == 'val':
            self.annot_path = path_config.VAL_JSON
        else:
            self.annot_path = path_config.TEST_JSON
    ...
    def __getitem__(self, index):
        ...
        file_name = self.coco.loadImgs(ids=[img_id])[0]['file_name']   # e.g. "Satellite_21_000001.jpg"
        ...
        # 解析文件名：Satellite_21_000001.jpg -> folder = "Satellite_21", frame_id = "000001"
        base = os.path.splitext(file_name)[0]          # "Satellite_21_000001"
        parts = base.split('_')                        # ["Satellite", "21", "000001"]
        folder = f"{parts[0]}_{parts[1]}"              # "Satellite_21"
        frame_id = parts[2]                            # "000001"
        ext = os.path.splitext(file_name)[1]           # ".jpg"

        # 读取当前帧图片，获取尺寸
        img_path = os.path.join(self.img_dir, folder, 'img1', f"{frame_id}{ext}")
        ...
        for ii in range(seq_num):
            # 计算当前帧之前的第 ii 帧的帧号（从当前帧往前推）
            frame_idx = max(int(frame_id) - seq_num + ii + 1, 1)
            frame_str = f"{frame_idx:06d}"
            img_name = f"{frame_str}{ext}"
            img_full_path = os.path.join(self.img_dir, folder, 'img1', img_name)

            if ii <= int(frame_id) - 1:
                temp_id = img_id - ii
            if ii != 0:
            ...
            if im is None:
                # 如果图片不存在，用黑色填充（理论上应该存在）
                im = np.zeros((im0.shape[0], im0.shape[1], 3), dtype=np.uint8)
        ...
        for ii in range(seq_num - 1):
            # 注意 pre_revrs_ids 的顺序是反的，需要调整索引
