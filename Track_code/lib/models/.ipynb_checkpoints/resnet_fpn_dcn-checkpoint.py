...
class DeConvLSTMNet(nn.Module):
    ...
    def forward(self, p0_sta, c0_lstm):
        ...
        for i in range(N - 1):
            if i == 0:
                final_lstm = self.proj0(torch.cat((p0_seq_lstm[:, i] + c0_lstm[:, i], p0_seq_lstm[:, i + 1] + c0_lstm[:, i + 1]), dim=1)).unsqueeze(1)
            else:
                final_temp = self.proj0(torch.cat((p0_seq_lstm[:, i] + c0_lstm[:, i], p0_seq_lstm[:, i + 1] + c0_lstm[:, i + 1]), dim=1)).unsqueeze(1)
                final_lstm = torch.cat((final_lstm, final_temp), dim=1)       # B, N - 1, 32, 512, 512
...
#                 # 如果 batch size 是 1，可以去掉批次维度

#                 ret_channel = p3[0, 220, :, :]
...
#                 plt.axis('off')  # 关闭坐标轴
#                 plt.savefig("poseResnet2/p3_220.png", bbox_inches='tight', pad_inches=0)
#                 plt.close()
                
#                 ret_channel = p2[0, 100, :, :]
...
#                 plt.title('Average Attention Map')
#                 plt.axis('off')  # 关闭坐标轴
#                 plt.savefig("poseResnet2/p2_100.png", bbox_inches='tight', pad_inches=0)
#                 plt.close()
...
#                 plt.title('Average Attention Map')
#                 plt.axis('off')  # 关闭坐标轴
#                 plt.savefig("poseResnet2/p1_30.png", bbox_inches='tight', pad_inches=0)
#                 plt.close()
...
#                 plt.title('Average Attention Map')
#                 plt.axis('off')  # 关闭坐标轴
#                 plt.savefig("poseResnet2/p0_10.png", bbox_inches='tight', pad_inches=0)
#                 plt.close()
...
#            print(mask_out.shape)  # 查看mask_out的形状
...
#                 os.makedirs('out')
# #            if B == 1:  # 当批量大小为 1 时，直接处理第一个图像
#             feature_map = mask_out[0, 0, 0].cpu().numpy()  # 提取第一个图像的特征图
#             # for i in range(B):
#             #     feature_map = mask_out[i, 0, 0].cpu().numpy()  # 提取第i张图像的特征图 [1, H, W]
#             if feature_map.shape == (H, W):
#                 plt.figure(figsize=(6, 6))
#                 plt.imshow(feature_map, cmap='viridis')  # 使用彩色热图
#                 plt.colorbar()  # 添加颜色条
#                 plt.axis('off')  # 关闭坐标轴
#                 plt.savefig(f"out/mask_heatmap_{i}.png", bbox_inches='tight', pad_inches=0)
#                 plt.close()
