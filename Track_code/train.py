...
def main(opt):
    ...
    # 保存基础数据集引用
    base_s = train_loader.dataset.coco
    
    # 创建模型
    print('Creating model...')
    head = {'hm': COCO(opt, 'train').num_classes, 'wh': 2, 'reg': 2, 'dis': 2}
    ...
    for epoch in range(start_epoch + 1, opt.num_epochs + 1):
        ...
        if val_intervals > 0 and epoch % val_intervals == 0:
            save_model(os.path.join(opt.save_dir, 'model_{}.pth'.format(epoch)),
                       epoch, model, optimizer)
            with torch.no_grad():
                log_dict_val, preds, stats = trainer.val(epoch, val_loader,base_s, COCO(opt, 'val'))
